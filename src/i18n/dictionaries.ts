export const dictionaries = {
  es: {
    hero: {
      title: 'Puerta de Bisagra',
      subtitle: 'La puerta más magnífica de la antigua ciudad de Toledo',
      rating: '4.7/5 (11,640 opiniones)',
      hours: 'Abierto 24 horas',
      tags: ['Puerta de la ciudad', 'Origen morisco', 'Escudo de Carlos V', 'Murallas medievales'],
    },
    intro: {
      text: 'La Puerta de Bisagra (también conocida como Puerta de Bisagra Nueva) es una de las puertas más majestuosas e icónicas de la ciudad de Toledo, España. Sirve como la entrada principal al casco antiguo y es un vívido testimonio del estatus de Toledo como la "Ciudad de las Tres Culturas".',
      bullets: [
        'Entrada principal desde Madrid o estaciones de tren/autobús',
        'Origen árabe del siglo X, reconstruida en estilo renacentista en 1559',
        'Impresionante escudo del águila bicéfala del emperador Carlos V',
        'Accesible para sillas de ruedas, ideal para niños',
      ],
    },
    reviews: {
      title: 'Opiniones de visitantes reales',
      declaration: 'Valoraciones y opiniones de Google Maps (última actualización: 2026).\nSolo mostramos una selección de opiniones verificadas con altas puntuaciones. Para ver todas las opiniones completas y más recientes, haga clic en el enlace de abajo.',
      moreText: 'Ver más opiniones en Google Maps',
      items: [
        { name: 'Carlos M.', date: 'Hace 2 meses', rating: 5, text: 'Una entrada monumental que te transporta en el tiempo. Obligatorio tomarse una foto aquí antes de entrar a la ciudad vieja.' },
        { name: 'Ana G.', date: 'Hace 4 meses', rating: 5, text: 'Arquitectura impresionante. El escudo de armas es gigante y muy detallado. De noche con la iluminación es aún más espectacular.' },
        { name: 'David R.', date: 'Hace 6 meses', rating: 5, text: 'Majestuosa. La mezcla de estilos defensivo y renacentista es perfecta. Es la mejor bienvenida a Toledo.' },
        { name: 'Elena V.', date: 'Hace 1 año', rating: 5, text: 'Un lugar lleno de historia. Puedes imaginar a los reyes pasando por debajo de sus arcos.' },
        { name: 'Juan P.', date: 'Hace 1 año', rating: 5, text: 'Fácil de llegar, imponente. El patio de armas interior es muy bonito y tranquilo.' },
      ]
    },
    map: {
      title: 'Ubicación y Mapa',
      openLink: 'Abrir en Google Maps'
    },
    footer: {
      support: 'Para soporte técnico de este sitio web, por favor contacte:',
      copyright: '© 2026 Puerta de Bisagra · Todos los derechos reservados.',
      dataRef: 'Datos y Referencias: Información extraída de Google Maps y recursos públicos. Fotos de Unsplash.'
    },
    nav: {
      privacy: 'Política de Privacidad',
      terms: 'Términos de Servicio',
      cookies: 'Configuración de Cookies'
    }
  },
  'zh-Hant': {
    hero: {
      title: 'Puerta de Bisagra | 比薩格拉門',
      subtitle: '西班牙托萊多古城最宏偉的城門',
      rating: '4.7/5 (11,640 評價)',
      hours: '24小時開放',
      tags: ['城門', '摩爾人起源', '查理五世徽章', '中世紀城牆'],
    },
    intro: {
      text: 'Puerta de Bisagra（比薩格拉門，也稱新比薩格拉門）是西班牙托萊多古城最宏偉、最具代表性的城門之一。它不僅是進入老城區的標誌性入口，更是托萊多作為「三文化之城」的生動見證。',
      bullets: [
        '從馬德里方向或火車站/汽車站進入古城的必經之路',
        '源於10世紀阿拉伯統治時期，1559年文藝復興風格重建',
        '外側雕刻有神聖羅馬帝國皇帝查理五世的巨大雙頭鷹徽章',
        '輪椅可通行，適合兒童遊覽（無障礙）',
      ],
    },
    reviews: {
      title: '真實遊客評價',
      declaration: '評分和評價來自 Google 地圖（最後更新：2026年）。\n我們僅展示經過驗證的精選高分評價。如需查看所有完整和最新評價，請點擊下方連結。',
      moreText: '在 Google Maps 上查看更多評價',
      items: [
        { name: '王小明', date: '2個月前', rating: 5, text: '非常宏偉的城門！雙頭鷹徽章極具氣勢，是進入托萊多必拍的打卡點。' },
        { name: 'Lin C.', date: '4個月前', rating: 5, text: '夜晚打燈後別有一番風味。穿過城門的小廣場很安靜，能感受到濃厚的歷史氣息。' },
        { name: '張先生', date: '6個月前', rating: 5, text: '保存得非常完好，軍事防禦與文藝復興藝術的完美結合。' },
        { name: 'Chen H.', date: '1年前', rating: 5, text: '站在門下能感覺到自己的渺小，西班牙帝國的榮光依稀可見。' },
        { name: '李小姐', date: '1年前', rating: 5, text: '交通很方便，從火車站走過來第一眼就看到它，震撼！' },
      ]
    },
    map: {
      title: '位置與地圖',
      openLink: '在 Google Maps 中打開'
    },
    footer: {
      support: '如需本網站的技術支持，請聯繫：',
      copyright: '© 2026 Puerta de Bisagra · 保留所有權利。',
      dataRef: '數據與參考：信息提取自 Google Maps 與公共資源。照片來源 Unsplash。'
    },
    nav: {
      privacy: '隱私政策',
      terms: '服務條款',
      cookies: 'Cookie 設定'
    }
  },
  en: {
    hero: {
      title: 'Puerta de Bisagra',
      subtitle: 'The most magnificent gate of Toledo\'s ancient city',
      rating: '4.7/5 (11,640 reviews)',
      hours: 'Open 24 hours',
      tags: ['City Gate', 'Moorish Origin', 'Charles V Coat of Arms', 'Medieval Walls'],
    },
    intro: {
      text: 'Puerta de Bisagra (also known as Puerta de Bisagra Nueva) is one of the most majestic and iconic gates of the city of Toledo, Spain. It serves as the main entrance to the old city and is a vivid witness to Toledo\'s status as the "City of Three Cultures".',
      bullets: [
        'Main entrance from Madrid or the train/bus stations',
        '10th-century Moorish origins, rebuilt in Renaissance style in 1559',
        'Striking double-headed eagle coat of arms of Emperor Charles V',
        'Wheelchair accessible entrance, good for kids',
      ],
    },
    reviews: {
      title: 'Real Visitor Reviews',
      declaration: 'Ratings and reviews from Google Maps (last updated: 2026).\nWe only show a selection of verified high-scoring reviews. To see all complete and latest reviews, click the link below.',
      moreText: 'See more reviews on Google Maps',
      items: [
        { name: 'John D.', date: '2 months ago', rating: 5, text: 'A monumental entrance that transports you back in time. A must-take photo spot before entering the old city.' },
        { name: 'Sarah W.', date: '4 months ago', rating: 5, text: 'Stunning architecture. The coat of arms is giant and highly detailed. Even more spectacular at night with the lighting.' },
        { name: 'Michael T.', date: '6 months ago', rating: 5, text: 'Majestic. The perfect blend of defensive and Renaissance styles. The best welcome to Toledo.' },
        { name: 'Emma R.', date: '1 year ago', rating: 5, text: 'A place full of history. You can almost imagine the kings passing beneath its arches.' },
        { name: 'David L.', date: '1 year ago', rating: 5, text: 'Easy to reach, imposing. The inner courtyard is very beautiful and peaceful.' },
      ]
    },
    map: {
      title: 'Location & Map',
      openLink: 'Open in Google Maps'
    },
    footer: {
      support: 'For technical support regarding this website, please contact:',
      copyright: '© 2026 Puerta de Bisagra · All rights reserved.',
      dataRef: 'Data & References: Information extracted from Google Maps and public resources. Photos from Unsplash.'
    },
    nav: {
      privacy: 'Privacy Policy',
      terms: 'Terms of Service',
      cookies: 'Cookie Settings'
    }
  },
  fr: {
    hero: {
      title: 'Puerta de Bisagra',
      subtitle: 'La porte la plus magnifique de la vieille ville de Tolède',
      rating: '4.7/5 (11 640 avis)',
      hours: 'Ouvert 24h/24',
      tags: ['Porte de ville', 'Origine mauresque', 'Armoiries de Charles Quint', 'Murailles médiévales'],
    },
    intro: {
      text: 'La Puerta de Bisagra (également connue sous le nom de Puerta de Bisagra Nueva) est l\'une des portes les plus majestueuses et emblématiques de la ville de Tolède, en Espagne. Elle sert d\'entrée principale à la vieille ville et témoigne de manière vivante du statut de Tolède en tant que « Ville des Trois Cultures ».',
      bullets: [
        'Entrée principale depuis Madrid ou les gares (train/bus)',
        'Origines mauresques du 10ème siècle, reconstruite en style Renaissance en 1559',
        'Impressionnantes armoiries de l\'aigle à deux têtes de l\'empereur Charles Quint',
        'Entrée accessible en fauteuil roulant, adapté aux enfants',
      ],
    },
    reviews: {
      title: 'Avis de vrais visiteurs',
      declaration: 'Évaluations et avis provenant de Google Maps (dernière mise à jour : 2026).\nNous ne montrons qu\'une sélection d\'avis hautement notés vérifiés. Pour voir tous les avis complets et les plus récents, cliquez sur le lien ci-dessous.',
      moreText: 'Voir plus d\'avis sur Google Maps',
      items: [
        { name: 'Pierre L.', date: 'Il y a 2 mois', rating: 5, text: 'Une entrée monumentale qui vous transporte dans le temps. Un arrêt photo obligatoire avant d\'entrer dans la vieille ville.' },
        { name: 'Marie C.', date: 'Il y a 4 mois', rating: 5, text: 'Architecture magnifique. Les armoiries sont géantes et très détaillées. Encore plus spectaculaire la nuit avec l\'éclairage.' },
        { name: 'Luc B.', date: 'Il y a 6 mois', rating: 5, text: 'Majestueux. Le mélange parfait de styles défensif et Renaissance. Le meilleur accueil à Tolède.' },
        { name: 'Sophie D.', date: 'Il y a 1 an', rating: 5, text: 'Un lieu chargé d\'histoire. On peut presque imaginer les rois passer sous ses arches.' },
        { name: 'Julien M.', date: 'Il y a 1 an', rating: 5, text: 'Facile d\'accès, imposant. La cour intérieure est très belle et paisible.' },
      ]
    },
    map: {
      title: 'Emplacement et Carte',
      openLink: 'Ouvrir dans Google Maps'
    },
    footer: {
      support: 'Pour le support technique concernant ce site web, veuillez contacter :',
      copyright: '© 2026 Puerta de Bisagra · Tous droits réservés.',
      dataRef: 'Données et Références : Informations extraites de Google Maps et de ressources publiques. Photos de Unsplash.'
    },
    nav: {
      privacy: 'Politique de confidentialité',
      terms: 'Conditions d\'utilisation',
      cookies: 'Paramètres des cookies'
    }
  }
};
